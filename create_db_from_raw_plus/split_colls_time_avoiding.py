...
def change_to_probe_gt_list(gt_obj):
    p_gt_l=[]
    for p in gt_obj:
        tmp_d={'probe_name':p,'gt':gt_obj[p]}
        p_gt_l.append(tmp_d)
    return p_gt_l
...
for g_obj in gt_find:
    ...
    del g_obj['Name']   # remove Name field and leave only probe genotypes
    gt_idx_f=change_to_probe_gt_list(g_obj)
    gt_array_len=len(g_obj)
